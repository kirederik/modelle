...
class Product < ActiveRecord::Base
  ...
  def verify_feedstock
    ...
    feedstocks.each do |f|
      ...
      if stock == nil
        status = status + " #{self.name} precisa de #{f.feedstock.name} que não está cadastrada no estoque."
      else 
        if stock.quantity < f.quantity
          status = status + " Não há #{stock.feedstock.name} suficiente para produzir #{self.name}"
        end
      end
    end
    status
